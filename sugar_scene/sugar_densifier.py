...
class SuGaRDensifier:
    ...
    def __init__(
        ...
        self.points_gradient_accum = torch.zeros(
            (self.model.points.shape[0], 1), device=self.model.device
        )
        self.denom = torch.zeros(
            (self.model.points.shape[0], 1), device=self.model.device
        )
        self.max_radii2D = torch.zeros(
            (self.model.points.shape[0]), device=self.model.device
        )
        ...
        if not self.model.freeze_gaussians:
            self.params_to_densify.extend(
                ["points", "all_densities", "scales", "quaternions"]
            )
            self.params_to_densify.extend(["sh_coordinates_dc", "sh_coordinates_rest"])
            self.params_to_densify.extend(
                ["features_base_color", "features_roughness"]
            )  # Add new features
    ...
    def prune_points(self, mask):
        ...
        if "features_base_color" in self.params_to_densify:
            self.model._features_base_color = optimizable_tensors["features_base_color"]
        if "features_roughness" in self.params_to_densify:
            self.model._features_roughness = optimizable_tensors["features_roughness"]
        ...
        for group in self.optimizer.param_groups:
            ...
            if name in self.params_to_densify:
                ...
                if stored_state is not None:
                    ...
                    group["params"][0] = nn.Parameter(
                        torch.cat(
                            (group["params"][0], extension_tensor), dim=dim_to_cat
                        ).requires_grad_(True)
                    )
                    self.optimizer.state[group["params"][0]] = stored_state
    ...
    def densification_postfix(
        ...
        new_scales,
        new_quaternions,
        new_sh_coordinates_dc=None,
        new_sh_coordinates_rest=None,
        new_features_base_color=None,
        new_features_roughness=None,
    ):
        tensors_dict = {
            "points": new_points,
        ...
        tensors_dict["features_base_color"] = new_features_base_color
        tensors_dict["features_roughness"] = new_features_roughness
    ...
    def update_densification_stats(
        self, viewspace_point_tensor, radii, visibility_filter
    ):
        ...
        self.points_gradient_accum[visibility_filter] += torch.norm(
    ...
    def densify_and_clone(self, grads, max_grad, extent):
        ...
        selected_pts_mask = torch.where(
            torch.norm(grads, dim=-1) >= max_grad, True, False
        )
        ...
        new_features_base_color = self.model._features_base_color[selected_pts_mask]
        new_features_roughness = self.model._features_roughness[selected_pts_mask]
        ...
        self.densification_postfix(
            ...
            new_features_base_color=new_features_base_color,
            new_features_roughness=new_features_roughness,
        )
    ...
    def densify_and_split(self, grads, max_grad, extent, N=2):
        ...
        rots = quaternion_to_matrix(self.model.quaternions[selected_pts_mask]).repeat(
            N, 1, 1
        )

        new_points = torch.bmm(rots, samples.unsqueeze(-1)).squeeze(
            -1
        ) + self.model.points[selected_pts_mask].repeat(N, 1)
        new_scales = self.model.scale_inverse_activation(
            self.model.scaling[selected_pts_mask].repeat(N, 1) / (0.8 * N)
        )
        ...
        new_sh_coordinates_dc = self.model._sh_coordinates_dc[selected_pts_mask].repeat(
            N, 1, 1
        )
        new_sh_coordinates_rest = self.model._sh_coordinates_rest[
            selected_pts_mask
        ].repeat(N, 1, 1)
        new_features_base_color = self.model._features_base_color[
            selected_pts_mask
        ].repeat(N, 1, 1)
        new_features_roughness = self.model._features_roughness[
            selected_pts_mask
        ].repeat(N, 1, 1)
        ...
        prune_filter = torch.cat(
            (
                selected_pts_mask,
                torch.zeros(
                    N * selected_pts_mask.sum(), device=self.model.device, dtype=bool
                ),
            )
        )
    ...
    def reset_opacity(self):
        opacities_new = inverse_sigmoid(
            torch.min(
                ...
                torch.ones_like(self.model.all_densities.view(-1, 1)) * 0.01,
            )
        )
        optimizable_tensors = self.replace_tensor_to_optimizer(
            opacities_new, "all_densities"
        )
        self.model.all_densities = optimizable_tensors["all_densities"]
