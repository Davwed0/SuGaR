...
class SuGaROptimizer:
    ...
    def __init__(
        ...
        if (not model.binded_to_surface_mesh and model.learn_positions) or (
            model.binded_to_surface_mesh and model.learn_surface_mesh_positions
        ):
            l = [
                {
                    "params": [model._points],
                    "lr": opt.position_lr_init * spatial_lr_scale,
                    "name": "points",
                }
            ]
        else:
        ...
        if not model.freeze_gaussians:
            l = l + [
                {
                    "params": [model._sh_coordinates_dc],
                    "lr": opt.feature_lr,
                    "name": "sh_coordinates_dc",
                },
                {
                    "params": [model._sh_coordinates_rest],
                    "lr": opt.feature_lr / 20.0,
                    "name": "sh_coordinates_rest",
                },
                {
                    "params": [model._features_base_color],
                    "lr": opt.feature_lr,
                    "name": "features_base_color",
                },
                {
                    "params": [model._features_roughness],
                    "lr": opt.feature_lr,
                    "name": "features_roughness",
                },
            ]
        ...
        if model.learn_opacities:
            l = l + [
                {
                    "params": [model.all_densities],
                    "lr": opt.opacity_lr,
                    "name": "all_densities",
                }
            ]
        if (model.binded_to_surface_mesh and model.learn_surface_mesh_scales) or (
            not model.binded_to_surface_mesh and model.learn_scales
        ):
            l = l + [
                {"params": [model._scales], "lr": opt.scaling_lr, "name": "scales"}
            ]
        if (model.binded_to_surface_mesh and model.learn_surface_mesh_scales) or (
            not model.binded_to_surface_mesh and model.learn_quaternions
        ):
            l = l + [
                {
                    "params": [model._quaternions],
                    "lr": opt.rotation_lr,
                    "name": "quaternions",
                }
            ]
