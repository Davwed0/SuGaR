...
def extract_mesh_and_texture_from_refined_sugar(args):
    ...
    if args.mesh_output_dir is None:
        if len(args.scene_path.split("/")[-1]) > 0:
            args.mesh_output_dir = os.path.join(
                "./output/refined_mesh", args.scene_path.split("/")[-1]
            )
        else:
            args.mesh_output_dir = os.path.join(
                "./output/refined_mesh", args.scene_path.split("/")[-2]
            )
    ...
    CONSOLE.print(
        "Base colors:",
        nerfmodel.gaussians._features_base_color.min().item(),
        nerfmodel.gaussians._features_base_color.max().item(),
        nerfmodel.gaussians._features_base_color.mean().item(),
        nerfmodel.gaussians._features_base_color.std().item(),
        sep="   ",
    )
    CONSOLE.print(
        "Roughness:",
        nerfmodel.gaussians._features_roughness.min().item(),
        nerfmodel.gaussians._features_roughness.max().item(),
        nerfmodel.gaussians._features_roughness.mean().item(),
        nerfmodel.gaussians._features_roughness.std().item(),
        sep="   ",
    )
    ...
    with torch.no_grad():
        refined_sugar._features_base_color[...] = (
            nerfmodel.gaussians._features_base_color
        )  # Add base color
        refined_sugar._features_roughness[...] = (
            nerfmodel.gaussians._features_roughness
        )  # Add roughness

    CONSOLE.print(
        "Base colors:",
        refined_sugar._features_base_color.min().item(),
        refined_sugar._features_base_color.max().item(),
        refined_sugar._features_base_color.mean().item(),
        refined_sugar._features_base_color.std().item(),
        sep="   ",
    )
    CONSOLE.print(
        "Roughness:",
        refined_sugar._features_roughness.min().item(),
        refined_sugar._features_roughness.max().item(),
        refined_sugar._features_roughness.mean().item(),
        refined_sugar._features_roughness.std().item(),
        sep="   ",
    )
    ...
    if postprocess_mesh:
        ...
        with torch.no_grad():
            ...
            face_mask = refined_sugar.strengths[..., 0] > -1.0
            for i in range(postprocess_iterations):
                ...
                edges_neighbors = knn_points(
                    all_edges[face_mask].view(1, -1, 2).float(),
                    all_edges[face_mask].view(1, -1, 2).float(),
                    K=2,
                )
                # If all edges of a face appear in the list at least twice, then the face is inside the mesh
                is_inside = (edges_neighbors.dists[0][..., 1].view(-1, 3) < 0.01).all(
                    -1
                )
            ...
            face_mask[~face_mask.clone()] = (
                face_densities > postprocess_density_threshold
            )
            ...
            new_scales = nerfmodel.gaussians._scaling.reshape(
                len(face_mask), -1, 3
            )[face_mask].view(-1, 3)
            new_quaternions = nerfmodel.gaussians._rotation.reshape(
                len(face_mask), -1, 4
            )[face_mask].view(-1, 4)
            new_densities = refined_sugar.all_densities.reshape(
                len(face_mask), -1, 1
            )[face_mask].view(-1, 1)
            new_sh_coordinates_dc = nerfmodel.gaussians._features_dc.reshape(
                len(face_mask), -1, 1, 3
            )[face_mask].view(-1, 1, 3)
            new_sh_coordinates_rest = nerfmodel.gaussians._features_rest.reshape(
                len(face_mask), -1, 15, 3
            )[face_mask].view(-1, 15, 3)
            new_features_base_color = (
                nerfmodel.gaussians._features_base_color.reshape(
                    len(face_mask), -1, 1, 3
                )[face_mask].view(-1, 1, 3)
            )
            new_features_roughness = (
                nerfmodel.gaussians._features_roughness.reshape(
                    len(face_mask), -1, 1
                )[face_mask].view(-1, 1)
            )
            ...
            new_o3d_mesh.vertex_normals = o3d.utility.Vector3dVector(
                new_normals.cpu().numpy()
            )
            new_o3d_mesh.vertex_colors = o3d.utility.Vector3dVector(
                torch.ones_like(new_verts).cpu().numpy()
            )
            ...
            refined_sugar._features_base_color[...] = (
                new_features_base_color  # Add base color
            )
            refined_sugar._features_roughness[...] = (
                new_features_roughness  # Add roughness
            )
    ...
    with torch.no_grad():
        save_obj(
            ...
            texture_map=textured_mesh.textures.maps_padded()[0].clamp(0.0, 1.0),
        )

        # Save additional material textures
        if hasattr(textured_mesh, "texture_base_color"):
            print("Saving base color texture...")

            base_color_path = mesh_save_path.replace(".obj", "_base_color.png")
            base_color_img = (
                textured_mesh.texture_base_color.clamp(0.0, 1.0).cpu().numpy()
            )
            base_color_img = (base_color_img * 255).astype(np.uint8)
            imageio.imsave(base_color_path, base_color_img)
            CONSOLE.print(f"Base color texture saved at: {base_color_path}")

        if hasattr(textured_mesh, "texture_roughness"):
            print("Saving roughness texture...")

            roughness_path = mesh_save_path.replace(".obj", "_roughness.png")
            roughness_img = (
                textured_mesh.texture_roughness.clamp(0.0, 1.0).cpu().numpy()
            )
            roughness_img_rgb = (roughness_img * 255).astype(np.uint8)

            imageio.imsave(roughness_path, roughness_img_rgb[:, :, 0])
            CONSOLE.print(f"Roughness texture saved at: {roughness_path}")
