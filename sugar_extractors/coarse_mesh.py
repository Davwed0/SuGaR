...
def extract_mesh_from_coarse_sugar(args):
    ...
    surface_level_triangle_scale = 2.0  # 2.
    ...
    fg_bbox_factor = 1.0  # 1.
    bg_bbox_factor = 4.0  # 4.
    poisson_depth = (
        10  # 10 for most real scenes. 6 or 7 work well for most synthetic scenes
    )
    vertices_density_quantile = (
        0.1  # 0.1 for most real scenes. 0. works well for most synthetic scenes
    )
    decimate_mesh = True
    clean_mesh = True
    ...
    if args.mesh_output_dir is None:
        if len(args.scene_path.split("/")[-1]) > 0:
            args.mesh_output_dir = os.path.join(
                "./output/coarse_mesh", args.scene_path.split("/")[-1]
            )
        else:
            args.mesh_output_dir = os.path.join(
                "./output/coarse_mesh", args.scene_path.split("/")[-2]
            )
    ...
    with torch.no_grad():
        ...
        CONSOLE.print(
            "Opacities min/max/mean:",
            sugar.strengths.min(),
            sugar.strengths.max(),
            sugar.strengths.mean(),
        )
        n_quantiles = 10
        for i in range(n_quantiles):
            CONSOLE.print(
                f"Quantile {i/n_quantiles}:",
                sugar.strengths.quantile(i / n_quantiles).item(),
            )
    ...
    if not use_marching_cubes:
        if not use_centers_to_extract_mesh:
            ...
            for surface_level in surface_levels:
                surface_levels_outputs[surface_level] = {
                    ...
                    "pix_to_gaussians": torch.zeros(
                        0, dtype=torch.long, device=sugar.device
                    ),
            ...
            with torch.no_grad():
                ...
                for cam_idx in range(len(nerfmodel.training_cameras)):
                    if cam_idx % 30 == 0:
                        CONSOLE.print(
                            f"Processing frame {cam_idx}/{len(nerfmodel.training_cameras)}..."
                        )
                        for surface_level in surface_levels:
                            CONSOLE.print(
                    ...
                    point_depth = (
                        cameras_to_use.p3d_cameras[cam_idx]
                    ...
                    rgb = (
                        sugar.render_image_gaussian_rasterizer(
                    ...
                    with torch.no_grad():
                        if use_fast_method:
                            frame_surface_level_outputs = sugar.compute_level_surface_points_from_camera_fast(
                                ...
                                n_surface_points=2
                                * n_pts_per_frame,  # TODO: 2*n_pts_per_frame is safe to avoid empty pixels
                        ...
                        for surface_level in surface_levels:
                            img_surface_points = frame_surface_level_outputs[
                                surface_level
                            ]["intersection_points"]
                            surface_gaussian_idx = frame_surface_level_outputs[
                                surface_level
                            ]["gaussian_idx"]
                            img_surface_normals = frame_surface_level_outputs[
                                surface_level
                            ]["normals"]

                            if use_fast_method:
                                pixel_idx = frame_surface_level_outputs[surface_level][
                                    "pixel_idx"
                                ]
                                img_surface_colors = rgb.view(-1, 3)[pixel_idx]
                            else:
                                empty_pixels = frame_surface_level_outputs[
                                    surface_level
                                ]["empty_pixels"]
                                img_surface_colors = rgb.view(-1, 3)[~empty_pixels]

                            img_surface_view_directions = torch.nn.functional.normalize(
                                cameras_to_use.p3d_cameras[cam_idx].get_camera_center()
                                - img_surface_points
                            )
                            img_surface_pix_to_gaussians = surface_gaussian_idx.view(-1)

                            idx = torch.randperm(
                                len(img_surface_points), device=sugar.device
                            )[:n_pts_per_frame]
                            ...
                            surface_levels_outputs[surface_level]["view_directions"] = (
                                torch.cat(
                                    [
                                        ...
                                        img_surface_view_directions[idx],
                                    ],
                                    dim=0,
                                )
                            )
                            surface_levels_outputs[surface_level][
                                ...
                                [
                                    ...
                                    img_surface_pix_to_gaussians[idx],
                                ],
                                dim=0,
                            )
            ...
            for surface_level in surface_levels:
                ...
                if use_custom_bbox:
                    ...
                    fg_bbox_min_tensor = (
                        ...
                        * torch.ones(1, 3, device=sugar.device)
                    )
                    fg_bbox_max_tensor = (
                        fg_bbox_factor
                        * sugar.get_cameras_spatial_extent()
                ...
                if center_bbox:
                    _cameras_spatial_extent, _camera_average_xyz = (
                        sugar.get_cameras_spatial_extent(return_average_xyz=True)
                    )
                    with torch.no_grad():
                ...
                fg_mask = (surface_points[points_idx] > fg_bbox_min_tensor).all(
                    dim=-1
                ) * (surface_points[points_idx] < fg_bbox_max_tensor).all(dim=-1)
                if center_bbox:
                    bg_mask = (
                        (surface_points[points_idx] - _camera_average_xyz)
                        .abs()
                        .max(dim=-1)[0]
                        < bg_bbox_factor * _cameras_spatial_extent
                    ) * ~fg_mask
                else:
                    bg_mask = (
                        surface_points[points_idx].abs().max(dim=-1)[0]
                        < bg_bbox_factor * sugar.get_cameras_spatial_extent()
                    ) * ~fg_mask
                ...
                if fg_points.shape[0] > 0:
                    ...
                    fg_pcd.points = o3d.utility.Vector3dVector(
                        fg_points.double().cpu().numpy()
                    )
                    fg_pcd.colors = o3d.utility.Vector3dVector(
                        fg_colors.double().cpu().numpy()
                    )
                    fg_pcd.normals = o3d.utility.Vector3dVector(
                        fg_normals.double().cpu().numpy()
                    )
                    ...
                    o3d_fg_mesh, o3d_fg_densities = (
                        o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(
                            fg_pcd, depth=poisson_depth
                        )
                    )  # , width=0, scale=1.1, linear_fit=False)  # depth=10 should be the default value? 11 is good to (but it starts to make a big number of triangles)

                    if vertices_density_quantile > 0.0:
                        CONSOLE.print("Removing vertices with low densities...")
                        vertices_to_remove = o3d_fg_densities < np.quantile(
                            o3d_fg_densities, vertices_density_quantile
                        )
                ...
                if bg_points.shape[0] > 0:
                    ...
                    bg_pcd.points = o3d.utility.Vector3dVector(
                        bg_points.double().cpu().numpy()
                    )
                    bg_pcd.colors = o3d.utility.Vector3dVector(
                        bg_colors.double().cpu().numpy()
                    )
                    bg_pcd.normals = o3d.utility.Vector3dVector(
                        bg_normals.double().cpu().numpy()
                    )

                    # outliers removal
                    cl, ind = bg_pcd.remove_statistical_outlier(
                        nb_neighbors=20, std_ratio=20.0
                    )
                    CONSOLE.print("Cleaning Point Cloud...")
                    ...
                    o3d_bg_mesh, o3d_bg_densities = (
                        o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(
                            bg_pcd, depth=poisson_depth
                        )
                    )  # , width=0, scale=1.1, linear_fit=False)  # depth=10 should be the default value? 11 is good to (but it starts to make a big number of triangles)

                    if vertices_density_quantile > 0.0:
                        CONSOLE.print("Removing vertices with low densities...")
                        vertices_to_remove = o3d_bg_densities < np.quantile(
                            o3d_bg_densities, vertices_density_quantile
                        )
                ...
                for decimation_target in decimation_targets:
                    ...
                    if decimate_mesh:
                        if o3d_fg_mesh is not None:
                            ...
                            decimated_o3d_fg_mesh = (
                                o3d_fg_mesh.simplify_quadric_decimation(
                                    decimation_target
                                )
                            )
                        ...
                        if o3d_bg_mesh is not None:
                            ...
                            decimated_o3d_bg_mesh = (
                                o3d_bg_mesh.simplify_quadric_decimation(
                                    decimation_target
                                )
                            )
                    ...
                    if (decimated_o3d_fg_mesh is not None) and (
                        decimated_o3d_bg_mesh is not None
                    ):
                        CONSOLE.print("Merging foreground and background meshes.")
                        decimated_o3d_mesh = (
                            decimated_o3d_fg_mesh + decimated_o3d_bg_mesh
                        )
                    elif decimated_o3d_fg_mesh is not None:
                        CONSOLE.print(
                            "Using foreground mesh only, since background mesh is empty."
                        )
                    ...
                    if project_mesh_on_surface_points:
                        ...
                        mesh_verts = torch.tensor(
                            np.asarray(decimated_o3d_mesh.vertices),
                            device=sugar.device,
                            dtype=torch.float32,
                        )
                        mesh_faces = torch.tensor(
                            np.asarray(decimated_o3d_mesh.triangles),
                            device=sugar.device,
                            dtype=torch.int64,
                        )
                        ...
                        decimated_o3d_mesh.vertices = o3d.utility.Vector3dVector(
                            new_mesh_verts.cpu().numpy()
                        )
                        decimated_o3d_mesh.triangles = o3d.utility.Vector3iVector(
                            new_mesh_faces.cpu().numpy()
                        )
                        decimated_o3d_mesh.vertex_colors = o3d.utility.Vector3dVector(
                            new_mesh_colors.cpu().numpy()
                        )
                    ...
                    if use_vanilla_3dgs:
                        ...
                        sugar_mesh_path = (
                            "sugarmesh_"
                            + sugar_checkpoint_path.split("/")[-2].replace(
                                "sugarcoarse_", ""
                            )
                    ...
                    ).replace("AA", str(decimation_target).replace(".", ""))
            ...
            CONSOLE.print(
                "Results will look bad, this is not the best way to extract a mesh."
            )
            ...
            with torch.no_grad():
                ...
                for decimation_target in decimation_targets:
                    ...
                    if use_vanilla_3dgs:
                        sugar_mesh_path = (
                            "sugarmesh_vanilla3dgs_poissoncenters_decimAA.ply"
                        )
                    else:
                        sugar_mesh_path = (
                            "sugarmesh_"
        ...
        with torch.no_grad():
            ...
            vertices, triangles = mcubes.marching_cubes(
                densities.cpu().numpy(), density_th
            )
            verts = (
                -sugar.get_cameras_spatial_extent()
                + (torch.tensor(vertices) / resolution)
                * 2
                * sugar.get_cameras_spatial_extent()
            )
            faces = torch.tensor(triangles.tolist())
            closest_gaussians = sugar.get_gaussians_closest_to_samples(
                verts.float().to(sugar.device)
            )
            verts_colors = SH2RGB(
                sugar._sh_coordinates_dc[closest_gaussians[:, 0]][:, 0, :]
            )
            ...
            mc_mesh.vertex_colors = o3d.utility.Vector3dVector(
                verts_colors.cpu().numpy()
            )
        ...
        with torch.no_grad():
            ...
            densities[
                (pts > -sugar.get_cameras_spatial_extent()).all(dim=-1)
                * (pts < sugar.get_cameras_spatial_extent()).all(dim=-1)
            ] = 0.0
            densities = densities.reshape(resolution, resolution, resolution)
            ...
            density_th = surface_levels[0]  # 1.
            CONSOLE.print(f"Computing mesh for surface level {density_th}...")
            bg_vertices, bg_triangles = mcubes.marching_cubes(
                densities.cpu().numpy(), density_th
            )
            bg_verts = (
                -4 * sugar.get_cameras_spatial_extent()
                + (torch.tensor(bg_vertices) / resolution)
                * 2
                * 4
                * sugar.get_cameras_spatial_extent()
            )
            ...
            closest_gaussians = sugar.get_gaussians_closest_to_samples(
                bg_verts.float().to(sugar.device)
            )
            bg_verts_colors = SH2RGB(
                sugar._sh_coordinates_dc[closest_gaussians[:, 0]][:, 0, :]
            )
            ...
            bg_mc_mesh.vertex_colors = o3d.utility.Vector3dVector(
                bg_verts_colors.cpu().numpy()
            )
        ...
        if decimate_mesh:
            ...
            decimated_o3d_fg_mesh = mc_mesh.simplify_quadric_decimation(
                decimation_target
            )
            print("Finished decimating mesh.")

            print("Decimating mesh...")
            decimated_o3d_bg_mesh = bg_mc_mesh.simplify_quadric_decimation(
                decimation_target
            )
            print("Finished decimating mesh.")
        else:
        ...
        if use_vanilla_3dgs:
            ...
            sugar_mesh_path = (
                "sugarmesh_"
                + sugar_checkpoint_path.split("/")[-2].replace("sugarcoarse_", "")
