    ...
    if initialize_from_trained_3dgs:
        with torch.no_grad():
            ...
            if prune_at_start:
                ...
                sugar._features_base_color[...] = (
                    nerfmodel.gaussians._features_base_color.detach()[start_prune_mask]
                )
                sugar._features_roughness[...] = (
                    nerfmodel.gaussians._features_roughness.detach()[start_prune_mask]
                )
            else:
                sugar._scales[...] = nerfmodel.gaussians._scaling.detach()
                sugar._quaternions[...] = nerfmodel.gaussians._rotation.detach()
                ...
                sugar._features_base_color[...] = (
                    nerfmodel.gaussians._features_base_color.detach()
                )
                sugar._features_roughness[...] = (
                    nerfmodel.gaussians._features_roughness.detach()
                )
        ...
        for i in range(0, train_num_images, train_num_images_per_batch):
            ...
            if iteration == 1 or iteration % print_loss_every_n_iterations == 0:
                ...
                with torch.no_grad():
                    ...
                    CONSOLE.print(
                        "Base colors:",
                        sugar._features_base_color.min().item(),
                        sugar._features_base_color.max().item(),
                        sugar._features_base_color.mean().item(),
                        sugar._features_base_color.std().item(),
                        sep="   ",
                    )
                    CONSOLE.print(
                        "Roughness:",
                        sugar._features_roughness.min().item(),
                        sugar._features_roughness.max().item(),
                        sugar._features_roughness.mean().item(),
                        sugar._features_roughness.std().item(),
                        sep="   ",
                    )
                    if regularize_sdf and iteration > start_sdf_regularization_from:
                        CONSOLE.print(
